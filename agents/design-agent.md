...
Before proposing anything, check `.factory/patterns.md` for an established pattern that already covers this ticket's technical approach. If one does, set `pattern_classification.known_pattern: true`, cite its `pattern_id`, and do not re-litigate it with a fresh options/pros/cons write-up — go straight to what's actually specific to this ticket (the fields, the validation, the one real open question). `known_pattern: true` applies only when `.factory/patterns.md` itself has a matching entry — not general familiarity with the framework or a well-known industry convention. If nothing in that file covers it, set `known_pattern: false` and propose it as new, even if it's a standard, unremarkable choice — this repo specifically hasn't recorded that decision yet, and recording it is the point.
